...
(function(a) {
	a.jBox = function(b, c) {
		...
		var d = (a.support.msie && parseInt(a.support.version) < 0x7);
		var e = a('#' + c.id);
		if (e.length > 0x0) {
		...
		if (!c.isTip && !c.isMessager && !c.showScrolling) {
			a(a.support.msie ? 'html' : 'body').attr('style', 'overflow:hidden;padding-right:17px;')
		};
		...
		if (c.showFade) {
			if ((d && a('iframe').length > 0x0) || a('object, applet').length > 0x0) {
				j.push('<iframe id="jbox-fade" class="jbox-fade" src="about:blank" style="display:block;position:absolute;z-index:-1;opacity:0.7!important;"></iframe>')
			} else {
				if (d) {
					a('select').css('visibility', 'hidden')
				};
				j.push('<div id="jbox-fade" class="jbox-fade" style="position:absolute;opacity:0.7!important;"></div>')
			}
		};
		...
		j.push('<div class="jbox-help-title jbox-title-panel" style="height:40px;display:none;"></div>');
		...
		j.push('<a class="jbox-close" title="' + a.jBox.languageDefaults.close + '" onmouseover="$(this).addClass(\'jbox-close-hover\');" onmouseout="$(this).removeClass(\'jbox-close-hover\');" style="position:absolute; display:block; cursor:pointer; top:12px; right:15px; width:15px; height:15px;' + (c.showClose ? '' : 'display:none;') + '"><i class=\'fa fa-times\'></i></a>');
		if (g) {
			j.push('<div class="jbox-title-panel" style="height:40px;">');
			j.push('<div class="jbox-title' + (c.showIcon == true ? ' jbox-title-icon' : (c.showIcon == false ? '' : ' ' + c.showIcon)) + '" style="float:left; width:' + i + '; line-height:40px; padding-left:' + (c.showIcon ? 0x12 : 0x5) + 'px;overflow:hidden;text-overflow:ellipsis;word-break:break-all;">' + (c.title == '' ? '&nbsp;' : c.title) + '</div>');
			j.push('</div>')
		};
		...
		var u = a.support.msie ? 'line-height:19px;padding:0px 6px 0px 6px;' : 'padding:0px 10px 0px 10px;';
		a.each(b, function(N, O) {
			...
			r.push('<div class="jbox-button-panel" style="height:30px;padding:10px;text-align: center;' + (P ? '' : 'display:none;') + '">');
			if (!c.isTip) {
		...
		var I = function() {
				if (!c.isTip && !c.isMessager) {
					if (a('.jbox-body').length == 0x1) {
						a(a.support.msie ? 'html' : 'body').removeAttr('style')
					};
					J()
				} else {
					if (c.isTip) {
		...
		if (!f.isObject) {
			...
			case "POST":
				a.ajax({
					...
					error: function() {
						o.find('#jbox-content-loading').html('<div style="padding-top:25px;padding-bottom:25px;text-align:center;">Loading Error.</div>')
					}
				});
				break;
			case "IFRAME":
	...
	a.jBox.prompt = function(b, c, d, e) {
		...
		var g = e.icon == 'none' ? 'height:auto;' : 'min-height:30px;' + ((a.support.msie && parseInt(a.support.version) < 0x7) ? 'height:auto !important;height:100%;_height:30px;' : 'height:auto;');
		var h = [];
		h.push('html:');
		h.push('<div style="margin:10px;' + g + 'padding-left:' + (e.icon == 'none' ? 0x0 : 0x28) + 'px;text-align:left;">');
		h.push('<span class="jbox-icon jbox-icon-' + e.icon + '" style="position:absolute; top:50px;left:' + (0xa + e.border) + 'px; width:32px; height:32px;"></span>');
		h.push(e.content);
		h.push('</div>');
		e.content = h.join('');
		a.jBox(e.content, e)
	...
	a.jBox.messager = function(b, c, d, e) {
		...
		var h = e.icon == 'none' ? 'height:auto;' : 'min-height:30px;' + ((a.support.msie && parseInt(a.support.version) < 0x7) ? 'height:auto !important;height:100%;_height:30px;' : 'height:auto;');
		var i = [];
		i.push('html:');
		i.push('<div style="margin:10px;' + h + 'padding-left:' + (e.icon == 'none' ? 0x0 : 0x28) + 'px;text-align:left;">');
		i.push('<span class="jbox-icon jbox-icon-' + e.icon + '" style="position:absolute; top:45px;left:' + (0xa + e.border) + 'px; width:32px; height:32px;"></span>');
		i.push(e.content);
		i.push('</div>');
		e.content = i.join('');
		a.jBox(e.content, e)
