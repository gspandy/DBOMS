			...
			if(result && result.list && result.list.length > 0){
				...
				if(bigProductObject){
					...
					for(var p in bigProductObject){
						...
						htmlResult += "<div class=\"rightt\"><label><input class=\"checkbox\" name=\"chkGoodsSku\" type=\"checkbox\" value=\""+p+"\" checked=\"checked\" desc=\""+bigProductObject[p]+"\" onclick=\"return false;\" style=\"display:none;\">"+bigProductObject[p]+"</label>"+
					...
					if(index > 0 && currentExtMarkCode == contractTerminalExtMarkCode){
						showDivCondition();
