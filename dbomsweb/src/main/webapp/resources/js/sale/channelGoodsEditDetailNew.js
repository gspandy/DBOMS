			...
			if(result && result.list && result.list.length > 0){
				for(var i = 0;i < result.list.length;i++){
					...
					if(currentExtMarkCode == contractTerminalExtMarkCode){
						...
						htmlResult += "<div class=\"rightt\"><label><input class=\"checkbox\" name=\"chkGoodsSku\" type=\"checkbox\" value=\""+productId+"\" checked=\"checked\" onclick=\"return false;\" style=\"display:none;\">"+productName+"</label>"+
							"<input id=\"txtGoodsName"+productId+"\" name=\"txtGoodsName"+productId+"\" type=\"text\" value=\""+goodsPrice+"\" style=\" text-align:right;width:80px;border: 0px;\" readonly />&nbsp;美元"+
							"&nbsp;&nbsp;( 海币限定 <input id=\"txtGoodsVirtual"+productId+"\" name=\"txtGoodsVirtual"+productId+"\" type=\"text\" value=\""+virtualPrice+"\" style=\" text-align:right;width:80px;border: 0px;\" readonly />&nbsp;分)";
...
//                        }
						...
						htmlResult += "<li><div class=\"leftt\">"+divLabel+"</div>";
						htmlResult += "<div class=\"rightt\"><label><input class=\"checkbox\" name=\"chkGoodsSku\" type=\"checkbox\" value=\""+goodsId+"\" checked=\"checked\" onclick=\"return false;\" style=\"display:none;\">"+productName+"</label>"+
							"<input id=\"txtGoodsName"+goodsId+"\" name=\"txtGoodsName"+goodsId+"\" type=\"text\" value=\""+goodsPrice+"\" style=\" text-align:right;width:80px;border: 0px;\" readonly />&nbsp;美元"+
							"&nbsp;&nbsp;( 海币限定 <input id=\"txtGoodsVirtual"+goodsId+"\" name=\"txtGoodsVirtual"+goodsId+"\" type=\"text\" value=\""+virtualPrice+"\" style=\" text-align:right;width:80px;border: 0px;\" readonly />&nbsp;分)";
						/*for (var a = 0; a < actExternalSysList.length; a++) {
				...
				if(result && result.list && result.list.length > 0){
					...
					$("span[name=spanPlanType]").click(function(e){
						...
						if(foundElements && foundElements.length > 0){
							...
							loadPackageProductHight(channelGoodsId , productId , productId2);
						}
						//移除其它的span的选中样式
...
function loadContractPlanLimit(productId , planTypeAttrVal){
	htmlResult = "<div class=\"lefttext\">Contract period</div><div class=\"righttext\">";
 ...
 * @return
 */
function loadPackageProductHight(channelGoodsId , productId1 , productId2){
	...
	htmlResult += "<strong>"+packageProductName+"</strong><a href=\"javascript:void(0);\" title=\"Hide/Expand\" style=\"width:80px;\" onclick=\"showOrHide('tblPackage"+productIdGroup+"')\" class=\"deleteCloss\">Hide/Expand</a>";
	...
	htmlResult += "</div>";
	htmlResult += "<br/><table class=\"packageTableStyle\" id=\"tblPackage"+productIdGroup+"\" width=\"100%\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\">";
	htmlResult += "<tr><th>Package monthly fee</th><th>Monthly return</th><th>Device payment</th><th>Registration return</th><th>Prepaid</th><th>Activities</th><th style=\"display:none;\">Whether to show</th></tr>";
			...
			if(result && result.list && result.list.length > 0){
				...
				for(var i = 0;i < result.list.length;i++){
					...
					htmlResult += "<tr>"+
						...
						"<td>"+pmProductObject[wcsDepositFee]+"</td><td>";
						var actExternalSysList = pmProductObject["actExternalSysList"];
                    for(var a=0;a<actExternalSysList.length;a++){
                    	htmlResult += "<a href='javascript:void(0);' onclick='toDetail(\"" + actExternalSysList[a]["actId"] + "\")'>" + actExternalSysList[a]["actName"] + "</a>";
                        if (a<(actExternalSysList.length-1)){
                            htmlResult += ",";
                        }
                    }
					htmlResult +=	"</td><td style=\"display:none;\"><input type=\"checkbox\" id=\"chkRecommended"+(productIdGroup+splitSymbol+pmProductObject["goodsId"])+"\" name=\"chkRecommended\" value=\""+pmProductObject["goodsId"]+"\" "+defaultSelectedStr+" onclick=\"return false;\"/></td></tr>";
				}
			}
...
/**
 * 判断指定的productId值goodsId在数组中是否存在
